...
async def edit_pwd(request:Request, user: user_dependency):
    ...
    return templates.TemplateResponse("edit-pwd.html", {"request": request, "user": user})

@router.post("/edit-pwd", response_class=HTMLResponse)
async def edit_user_pwd(request: Request, user: user_dependency, db: db_dependency,username: str = Form(...), password: str = Form(...), password2: str = Form(...)):
    if user is None:
        return RedirectResponse(url="/auth", status_code=status.HTTP_302_FOUND)
    if password == password2:
        return templates.TemplateResponse("edit-pwd.html", {"request": request, "user": user, "msg": "Passwords match"})
    
    user_model = db.query(User).filter(User.id == user.get("user_id")).first()
    msg = "Invalid username or password"

    if user_model is not None:
        if user_model.username == username and verify_password(password, user_model.password):
            user_model.password = get_password_hash(password2)
            db.add(user_model)
            db.commit()
            msg = "Password updated"

 
    return templates.TemplateResponse("edit-pwd.html", {"request": request, "user": user, "msg": msg, "user": user})
